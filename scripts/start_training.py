...
def main_worker(rank, opts, world_size, distributed):
    ...
    if opts.curriculum_training:
        ...
        current_stage = h_utils.get_current_stage(opts.start_epoch)
        labels_transform = h_utils.initialize_new_classification_layer_sum(model, current_stage)
        model.cuda(opts.gpu)
    ...
    if opts.evaluate is not None:
        ...
        if opts.evaluate != 'PGD-t-all':
            ...
            summary_val = eval(val_loader, model, loss_function, distances,
                                       ...
                                       h_utils=h_utils, save_adv=opts.save_adversaries)

            if opts.save_adversaries:
                summary_val, save_adversaries = summary_val
                torch.save(save_adversaries, get_name(opts) + '.pth')


            with open(get_name(opts) + '.json', "w") as fp:
...
def get_name(opts):
    ...
    if opts.evaluate == 'none':
        ...
        name = 'hPGD-u-{}-level{}-iter{:d}-eps{:d}-step{:d}-eval'.format(
        ...
        name = '{}-iter{:d}-eps{:d}-step{:d}-eval'.format(opts.evaluate,
                                                               opts.attack_iter,
                                                               int(255 * opts.attack_eps),
                                                               int(255 * opts.attack_step))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--evaluate", default=None, help='Evaluation protocol',
                        choices=['none', 'PGD-u', 'PGD-t', 'FAB', 'PGD-t-all', 'hPGD-u'])
    ...
    parser.add_argument("--hPGD", default='extra_max', type=str, help='Hierarchical PGD type',
                        choices=['extra_max', 'extra_topk', 'extra_mean', 'intra', 'extra_wo_h', 'extra_sum'])
    ...
    parser.add_argument("--save-adversaries", action='store_true', help='Store Adversaries')
