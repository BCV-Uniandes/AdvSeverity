...
output_path = 'RESULTS/freeVsCurr'
...
def plot_info(curr_data, free_data, r, metric):
    ...
    plt.plot(epses, [0 for _ in epses], 'k--', label='Free')
    plt.plot(epses, [0 for _ in epses], 'k-', label='Curriculum')

    for idx, m in enumerate(iters):
        curr = curr_data[m][r][metric]
        free = free_data[m][r][metric]
        plt.plot(epses, curr, '.' + colors[idx] + '-', label=f'm={m}')
        plt.plot(epses, free, '.' + colors[idx] + '--')
        ...
        for idx, eps in enumerate(epses):
            ...
            for r in results_to_check:
                ...
                try:
                    ...
                    print(f'Skipping {r + attack} in {exp_name}. No results available')
                    continue

    results[a] = data

# save all results
with open(f'{output_path}/tables-results.json', 'w') as f:
...
for a in steps:
    ...
    for r in results_to_check:
        ...
        plt.suptitle(r)
        plt.savefig(f'{output_path}/{r}_step{a}.png')
        plt.close()
