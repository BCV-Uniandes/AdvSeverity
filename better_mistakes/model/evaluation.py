...
from better_mistakes.model.init import hAutoAttackWrapper
...
def eval(loader, model, loss_function, distances, all_soft_labels,
         ...
         current_label=None,  # PGD-t-all
         h_utils=None,
         save_adv=False):  # hierarchy attack, args
    """
    ...
    for (embeddings, target) in tqdm(loader):
        ...
        with torch.no_grad():
            ...
            # if attack == 'hPGD-u':
                ...
                # output, target = h_utils.get_logits(output, target)
        ...
        #######################################################
        # AutoAttack evaluation step
        #######################################################

        elif attack == 'AutoAttack':
            adversary = AutoAttack(model, norm='Linf', eps=attack_eps, version='standard')
            x_adv = adversary.run_standard_evaluation(embeddings, target, bs=embeddings.size(0))
        ...
        #######################################################
            ...
            adversary.attacks_to_run = ['apgd-ce', 'apgd-t']  # the sqaure and fab attack is never successful
